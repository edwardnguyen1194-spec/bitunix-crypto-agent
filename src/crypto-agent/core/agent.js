...
class CryptoAgent extends EventEmitter {
  ...
  async _scan() {
    ...
    const mode = this.isDeadHours ? '🌙 LOW-VOL' : '☀️ ACTIVE';
    console.log(`\n─── Scan #${this.totalScans} @ ${timestamp} [${mode}] ───`);
    ...
    for (const symbol of this.config.symbols) {
      try {
        ...
        console.log(`  ${icon} ${symbol}: ${signal.action} | Score: ${signal.score.toFixed(1)} | Confluence: ${signal.confluence}/5`);
        ...
        if (adxValue > 25) {
          ...
          if ((signal.action === 'BUY' || signal.action === 'SELL') && signal.absScore > 50 && signal.confluence >= 4) {
            tradeAllowed = true;
          }
        }
